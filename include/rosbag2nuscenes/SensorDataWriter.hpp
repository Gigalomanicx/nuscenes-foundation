...
class SensorDataWriter {
    ...
    private:
        ...
        void writeRadarData(const RadarMessageT& msg, const fs::path& filename);

        void writeLidarData(const LidarMessageT& msg, const fs::path& filename);

        void writeCameraData(const CameraMessageT& msg, const fs::path& filename);
