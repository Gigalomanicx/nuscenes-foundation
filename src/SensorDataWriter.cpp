...
SensorDataWriter::SensorDataWriter(int num_workers) : file_queue_(MAX_QUEUE_SIZE), finished_(false) {
...
void SensorDataWriter::writeRadarData(const RadarMessageT& msg, const fs::path& filename) {
    if (fs::exists(filename)) {
        return;
    }
    pcl::io::savePCDFileBinary(filename.string(), msg.cloud);
}

void SensorDataWriter::writeLidarData(const LidarMessageT& msg, const fs::path& filename) {
    if (msg.cloud.empty()) {
...
void SensorDataWriter::writeCameraData(const CameraMessageT& msg, const fs::path& filename) {
    cv::imwrite(filename.string(), msg.image);
}
